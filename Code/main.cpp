    ...
    void display_image_in_new_window(const std::string& image_path) {
        Fl_RGB_Image* img_to_display = loadPPM(image_path.c_str());
        if (img_to_display) {
            Fl_RGB_Image* resized_img = resize_image(img_to_display, 400, 400); // Redimensionner à 400x400
            if (resized_img) {
                Fl_Window* img_window = new Fl_Window(420, 420, "Image Display");
                Fl_Box* img_box = new Fl_Box(10, 10, 400, 400);
                img_box->image(resized_img);
                img_window->end();
                img_window->show();
            } else {
                cerr << "Erreur : Échec du redimensionnement de l'image." << endl;
            }
            delete img_to_display; // Libérer l'image originale pour éviter les fuites mémoire
        } else {
    ...
    void process_image(Fl_Widget* w, void* data) {
        ...
        if (genererSLICC) {
            ...
            display_image_in_new_window(nomFichierSortieSLICC);
        }
        ...
        if (genererMeanShift) {
            ...
            display_image_in_new_window(nomFichierSortieMean);
        }
